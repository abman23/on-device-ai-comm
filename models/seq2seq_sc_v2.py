...
from .channels import AWGNModel, CDLModel, RealisticModel
...
class TFEncoderChannelModelOutput(TFBaseModelOutput):
    """Output of EncoderChannel that includes
        - semantic encoder
        - channel encoder
        - mapper
        - channel
...
@dataclass
class TFSeq2SeqSCModelOutput(TFSeq2SeqModelOutput):
    """Output of TFSeq2SeqSCMainLayer"""
    ber: Optional[tf.Tensor] = None

@dataclass
class TFSeq2SeqSCLMOutput(TFSeq2SeqLMOutput):
    """Output of TFSeq2SeqSCForConditionalGeneration"""
    ber: Optional[tf.Tensor] = None
...
class TFSeq2SeqSCMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        # If Training TFSeq2SeqSCForConditionalGeneration or not
        ...
        # Semantic Encoder
        self.semantic_encoder=bart_main_layer.encoder
        
        # Wireless Communication Channel(FEC and Channel)
        ...
        # Semantic Decoder
        self.semantic_decoder = bart_main_layer.decoder  
    ...
    def _setup_channel(self, channel_type, ch_config):
        if channel_type == 'AWGN':
            ...
            self.channel_model = RealisticModel(
                fec_type=ch_config['fec_type'],
            ...
            raise ValueError('Invalid Channel type. Channel type should be AWGN or CDL')
        # self._add_channel_noise = self.channel_model._add_channel_noise
    
    def _setup_bin_conv(self, bin_conv_method):
        self.bin_conv_method = bin_conv_method
        logger.info(f'{bin_conv_method=}')
        if bin_conv_method == 'tanh':
    ...
    def call(self,
             ...
             **kwargs) -> Union[TFSeq2SeqSCModelOutput, Tuple[tf.Tensor]]:
        ...
        #region - Channel
        # Reshape the output of semantic encoder
        shape = tf.shape(encoder_outputs.last_hidden_state)
        semantic_encoder_output = encoder_outputs.last_hidden_state

        # 2. VectorQuantizer layer
        # binarize (and get codebook indices if vector quantizing)
        for f in self._tensor_to_binary:
            semantic_encoder_output = f(semantic_encoder_output)
        semantic_encoder_output_binary = semantic_encoder_output

        # 3. Wireless Communication Channel(FEC and Channel)
        # add channel noise to binarized semantic encoder output
        last_hidden_state_binary = self.channel_model(semantic_encoder_output_binary)
        ...
        if not self.do_train:
            last_hidden_state_binary = tf.reshape(last_hidden_state_binary, tf.shape(semantic_encoder_output_binary))
            ber = get_ber(semantic_encoder_output_binary, last_hidden_state_binary)
        else:
            # While training, does not calculate BER.
            ber = tf.constant(-1.0, dtype=tf.float32)

        encoder_outputs = TFEncoderChannelModelOutput(
...
class TFSeq2SeqSCForConditionalGeneration(TFBartForConditionalGeneration):
    ...
    def get_encoder(self):
        return self.model.semantic_encoder
    ...
    def call(
        ...
        if self.bin_conv_method == 'vector_quantization' and masked_lm_loss is not None:
            ...
            #     vq_loss, 
            #     sep=',',
            #     output_stream='file:///home/danny911kr/joohan/seq2seq-sc2/loss.log')
